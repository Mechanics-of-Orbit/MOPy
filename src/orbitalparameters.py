...
sys.path.append("..\_methods")
from _methods.oprint import oprint
from _methods.custom_exceptions import TooManyInputs
...
class OrbitalValues:
    ...
    def __init__(self, orbit):
        self.orbit = orbit
        variables = {'sma': self.orbit.SemiMajorAxis, 'e': self.orbit.Eccentricity, 'ra': self.orbit.RadiusOfApogee, 'rp': self.orbit.RadiusOfPerigee}
        # check if variables has less than or equal two 2 values
        if sum(var is not None for var in variables.values()) <= 2:
            self.calculate_values_not_entered()
        else:
            raise TooManyInputs(
                "Enter any of the two values only to avoid over constrained situation", 
                **{str(var_names): vals for var_names, vals in variables.items()}
            )

    def calculate_orbital_constants(self):
        self.orbit.MajorBodyMass = pandet.value(self.orbit.MajorBody, "mass")
        self.orbit.GravitationalConstant = (
            GCs.NEWTON_GRAVITATIONAL_CONSTANT * self.orbit.MajorBodyMass
        )
        self.orbit.SemiLatusRectum = self.orbit.SemiMajorAxis * (
            1 - self.orbit.Eccentricity**2
        )
        self.orbit.SpecificAngularMomentum = (
            self.orbit.RadiusOfPerigee
            * (1 + self.orbit.Eccentricity)
            * self.orbit.GravitationalConstant
        ) ** 0.5
        self.orbit.TimePeriod = np.sqrt(
            ((4 * np.pi**2) / self.orbit.GravitationalConstant)
            * self.orbit.SemiMajorAxis**3
        )
        self.orbit.MeanMotion = self.orbit.TimePeriod / (2 * np.pi)
        self.orbit.SpecificMechanicalEnergy = -self.orbit.GravitationalConstant / (
            2 * self.orbit.SemiMajorAxis
        )

    def calculate_values_not_entered(self):
        if self.orbit.RadiusOfApogee and self.orbit.RadiusOfPerigee is not None:
            self.orbit.SemiMajorAxis = (
                self.orbit.RadiusOfPerigee + self.orbit.RadiusOfApogee
            ) / 2
            ...
            self.orbit.RadiusOfPerigee = self.orbit.SemiMajorAxis * (
                1 - self.orbit.Eccentricity
            )
            self.orbit.RadiusOfApogee = self.orbit.SemiMajorAxis * (
                1 + self.orbit.Eccentricity
            )

        elif self.orbit.SemiMajorAxis and self.orbit.RadiusOfApogee is not None:
            self.orbit.RadiusOfPerigee = (
                2 * self.orbit.SemiMajorAxis - self.orbit.RadiusOfApogee
            )
            self.orbit.Eccentricity = (
                self.orbit.RadiusOfApogee - self.orbit.RadiusOfPerigee
            ) / (self.orbit.RadiusOfApogee + self.orbit.RadiusOfPerigee)
            ...
            self.orbit.RadiusOfApogee = (
                2 * self.orbit.SemiMajorAxis - self.orbit.RadiusOfPerigee
            )
            self.orbit.Eccentricity = (
                self.orbit.RadiusOfApogee - self.orbit.RadiusOfPerigee
            ) / (self.orbit.RadiusOfApogee + self.orbit.RadiusOfPerigee)
            ...
            self.orbit.RadiusOfPerigee = self.orbit.RadiusOfApogee * (
                (1 - self.orbit.Eccentricity) / (1 + self.orbit.Eccentricity)
            )
            self.orbit.SemiMajorAxis = (
                self.orbit.RadiusOfPerigee + self.orbit.RadiusOfApogee
            ) / 2
    ...
    def calculate_velocities_at_these_angles(self, *angles):
        return [
            OrbitalValues.calculate_velocity_at_point(self, angle=angle)
            for angle in angles
        ]

    def calculate_velocities_at_these_points(self, *radii):
        return [
            OrbitalValues.calculate_velocity_at_point(self, radius=radius)
            for radius in radii
        ]
    ...
    def calculate_velocity_at_point(self, radius=None, angle=None):
        if angle is not None:
            radius = (
                self.SpecificAngularMomentum**2 / self.GravitationalConstant
            ) / (1 + self.Eccentricity * np.cos(angle))

        velocity = np.sqrt(
            (2 * self.GravitationalConstant / radius)
            - (self.GravitationalConstant / self.SemiMajorAxis)
        )
